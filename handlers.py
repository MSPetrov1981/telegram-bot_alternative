...
def _strip_data(obj: Any) -> Any:
    """Рекурсивно удаляет лишние пробелы из ключей и значений JSON."""
...
try:
    ...
    raise RuntimeError("Файл data.json не найден в директории бота.")

def cancel_keyboard() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup([[KeyboardButton("Отмена записи")]], resize_keyboard=True, one_time_keyboard=True)

async def _send_media_or_text(query, text: str, keyboard, photo_path: str | None = None) -> None:
    """Унифицированная отправка: фото+текст или редактирование сообщения."""
    if photo_path and await aiofiles_os.path.exists(photo_path):
        ...
        await query.edit_message_text(f"{text}\n\n📷 Фото не найдено" if not (photo_path and await aiofiles_os.path.exists(photo_path)) else text, reply_markup=keyboard, parse_mode="Markdown")

def is_valid_phone(phone: str) -> bool:
    cleaned = re.sub(r"[\s\-()]", "", phone)
    return bool(re.match(r"^\+?\d{7,}$", cleaned))
    ...
    for img in DATA["clinic_images"]:
        if await aiofiles_os.path.exists(img):
            async with aiofiles.open(img, "rb") as ph:
                media_group.append(InputMediaPhoto(media=await ph.read(), caption="Клиника" if not media_group else ""))
...
async def start_appointment_form(update: Update, context: ContextTypes.DEFAULT_TYPE, service_info: str | None = None) -> int:
    logger.info("Starting form, service_info=%s", service_info)
    context.user_data.clear()
    context.user_data["appointment_step"] = AWAITING_NAME
    context.user_data["skip_service_step"] = bool(service_info)
    if service_info:
...
async def button_appointment(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    return await start_appointment_form(update, context, service_info=get_service_or_doctor_name(parts[1], "_".join(parts[2:])))
...
async def send_email_notification(name: str, phone: str, service: str, comment: str) -> None:
    smtp_host, smtp_port, smtp_user, smtp_pass, email_to = (
        ...
        os.getenv("EMAIL_TO", "s1digital@ya.ru"),
    )
    ...
    msg = EmailMessage()
    msg["From"], msg["To"], msg["Subject"] = smtp_user, email_to, f"Новая заявка на запись от {name}"
    msg.set_content(f"Имя: {name}\nТелефон: {phone}\nУслуга/врач: {service}\nКомментарий: {comment or 'нет'}\n")

    try:
        async with SMTP(hostname=smtp_host, port=smtp_port, use_tls=(smtp_port == 465), start_tls=(smtp_port == 587)) as smtp:
...
async def process_appointment_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int | None:
    ...
    logger.info("Input step=%s, text='%s'", step, text[:50])
    ...
    if step == AWAITING_NAME:
        if len(text.split()) < 2:
            await update.message.reply_text("Введите полные ФИО (Имя и Фамилию):", reply_markup=cancel_keyboard())
            return AWAITING_NAME
        context.user_data.update({"full_name": text, "appointment_step": AWAITING_PHONE})
        await update.message.reply_text("Введите номер телефона (например, +7 123 456-78-90):", reply_markup=cancel_keyboard())
        return AWAITING_PHONE

    if step == AWAITING_PHONE:
        if not is_valid_phone(text):
            await update.message.reply_text("❌ Некорректный номер. Введите цифры, возможно с + или дефисами.", reply_markup=cancel_keyboard())
            return AWAITING_PHONE
        next_step = AWAITING_COMMENT if context.user_data.get("skip_service_step") else AWAITING_SERVICE
        context.user_data.update({"phone": text, "appointment_step": next_step})
        prompt = "Пожелания/комментарий (или «нет»):" if next_step == AWAITING_COMMENT else "К какому врачу/на какую услугу записаться?"
        await update.message.reply_text(prompt, reply_markup=cancel_keyboard())
        return next_step

    if step == AWAITING_SERVICE:
        if not text:
            await update.message.reply_text("Укажите услугу или врача.", reply_markup=cancel_keyboard())
            return AWAITING_SERVICE
        context.user_data.update({"service": text, "appointment_step": AWAITING_COMMENT})
        await update.message.reply_text("Пожелания/комментарий (или «нет»):", reply_markup=cancel_keyboard())
        return AWAITING_COMMENT

    if step == AWAITING_COMMENT:
        comment = "" if text.lower() in {"нет", "нету", "-", "—"} else text
        await send_email_notification(context.user_data["full_name"], context.user_data["phone"], context.user_data["service"], comment)
        await update.message.reply_text(
...
async def cancel_appointment_form(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    context.user_data.clear()
    await update.message.reply_text("Форма отменена.", reply_markup=main_menu_keyboard())

# ---------------------- INLINE ОБРАБОТЧИКИ ----------------------
async def _inline_base(update: Update, _: ContextTypes.DEFAULT_TYPE, data_path: list[str], key_extractor: str, kb_func: callable) -> None:
    query = update.callback_query
    await query.answer()
    key = query.data.removeprefix(key_extractor)
    obj = DATA
    for p in data_path:
        obj = obj[p]
    obj = obj[key]
    await _send_media_or_text(query, f"{obj.get('name', '')}\n{obj.get('specialization', '')}\n\n{obj.get('description', '')}", kb_func(key, *([obj.get('specialization')] if 'specialization' in key_extractor else [])), obj.get("photo"))

# Оптимизированные вызовы через универсальную логику:
async def button_doctor(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await _inline_base(update, context, ["specialists", "doctors"], "doctor_", doctor_detail_keyboard)

async def button_service_specialists(update: Update, _) -> None:
    query = update.callback_query; await query.answer()
    await query.edit_message_text("Выберите специализацию:", reply_markup=service_specializations_keyboard())

async def button_specialization(update: Update, _) -> None:
    query = update.callback_query; await query.answer()
    key = query.data.removeprefix("specialization_")
    await query.edit_message_text(f"Выберите врача ({DATA['specializations'][key]['title']}):", reply_markup=service_specialists_keyboard(key))

async def button_service_doctor(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query; await query.answer()
    ...
    for spec_key, spec in DATA["specializations"].items():
        if doc_key in spec["doctors"]:
            ...
            await _send_media_or_text(query, f"{doc['name']}\n{doc['specialization']}", service_doctor_detail_keyboard(doc_key, spec_key), doc.get("photo"))
            return

async def button_service_doctor_detail(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query; await query.answer()
...
async def button_service_procedures(update: Update, _) -> None:
    query = update.callback_query; await query.answer()
    await query.edit_message_text(DATA["procedures"]["title"], reply_markup=service_procedures_keyboard())

async def button_procedure(update: Update, _) -> None:
    query = update.callback_query; await query.answer()
...
async def button_direction(update: Update, _) -> None:
    query = update.callback_query; await query.answer()
...
async def button_direction_detail(update: Update, _) -> None:
    query = update.callback_query; await query.answer()
...
async def button_direction_more_detail(update: Update, _) -> None:
    query = update.callback_query; await query.answer()
...
async def button_diagnostic_detail(update: Update, _) -> None:
    query = update.callback_query; await query.answer()
...
async def button_service_what_we_treat(update: Update, _) -> None:
    query = update.callback_query; await query.answer()
    await query.edit_message_text(DATA["what_we_treat"]["title"], reply_markup=what_we_treat_keyboard())

async def button_treatment(update: Update, _) -> None:
    query = update.callback_query; await query.answer()
...
async def button_treatment_detail(update: Update, _) -> None:
    query = update.callback_query; await query.answer()
...
async def button_health_program(update: Update, _) -> None:
    query = update.callback_query; await query.answer()
...
async def button_health_program_detail(update: Update, _) -> None:
    query = update.callback_query; await query.answer()
...
def get_service_or_doctor_name(app_type: str, app_id: str) -> str:
    try:
        paths = {
            "procedure": ["procedures", "procedures_list"],
            "direction": ["directions", "directions_list"],
            "health_program": ["health_programs", "programs_list"],
            "diagnostic": ["diagnostics", "diagnostics_list"],
            "treatment": ["what_we_treat", "treatments_list"],
        }
        if app_type == "doctor":
            ...
            for spec in DATA["specializations"].values():
                if app_id in spec["doctors"]:
                    ...
                    return f"{d['name']} ({d['specialization']})"
        elif app_type in paths:
            obj = DATA
            for p in paths[app_type]:
                obj = obj[p]
